...
function CourseFile({ title, description, icon }: CourseFileProps) {
  return (
    <Link href={`notes/${title}`}>
      <div
        className={`p-6 w-full -mb-5 flex gap-4 items-center cursor-pointer transition-colors border border-gray-400 rounded-lg bg-fd-card hover:bg-fd-accent/50 hover:text-fd-accent-foreground backdrop-blur-md justify-between ${styles.cf} `}
      >
