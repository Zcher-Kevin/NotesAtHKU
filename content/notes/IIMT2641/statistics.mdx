...
#### Critical Region
- The range of values for the statistic that would reject $H_0$

#### Example – Finding Critical Values
$X \sim B(10, 0.4)$ at 5% significance level:
- $P(X \geq 7) = 0.0548 > \text{s.l.}$
- $P(X \geq 8) = 0.0132 < \text{s.l.}$

Conclusion:
- $c.r. = (X \geq 8)$
- $c.v. = 8$
- $a.s.l.$ ("Actual significance level") = $P(X \geq 8) = 0.0132$

For two-tailed tests, there are two critical regions at each end of the distribution. $a.s.l.$ = sum of $c.r.$ -->
