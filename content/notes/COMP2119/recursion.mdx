...
<Block variant="knowledge" title="Relevant mathematics">
    ...
    2. Arithmetic series: $S_n = n(\frac{a_1+a_n}{2}),     n=\text{number of terms}, a=\text{first / last term}$
...
For $T(n) = a\cdot T(\frac{n}{b}) + f(n),     T(1) = c,     a, c > 0,\ b > 1,\ f(n)\in \Theta(n^k),\ d\geq 0$, we have:
```math
T(n) = \begin{cases}
