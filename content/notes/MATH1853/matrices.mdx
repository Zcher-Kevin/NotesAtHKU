...
\end{smallmatrix}|=0:    (1-\lambda)^2 - 4 = 0    \rightarrow    \lambda_1=3,\ \lambda_2=-1$
</Block>
...
From the augmented matrix: $-2x_1+x_2+x_3=0,     -x_2+x_3=0$.
...
```math
A=VDV^{-1},    \text{where }V=\begin{bmatrix}
v_1 \\\dots\\v_n
...
Given $x(0)$, solve for $x(t)$ with the following steps:
    ...
    3. Solve c by augmented matrix $\begin{amatrix}{3}
v_1&\dots&v_n&x(0)
\end{amatrix}$
