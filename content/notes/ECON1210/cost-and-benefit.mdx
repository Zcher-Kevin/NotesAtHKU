...
!!! tip "Economic surplus and opportunity cost"
    ...
    Economic surplus, or net benefit, for taking an action $n$ is given by: $ES(n) = B(n) - C(n)$.

    **Opportunity cost** is the cost of choosing one action $n$ over another ($n_2$):

    $OC(n) = C(n) + ES(n_2)$

    The economic surplus of choosing $n$ over $n_2$ is:
...
!!! info "Decision by margin"
    ...
    **Only take action when $MB \geq MC$.**

    $MX(n) = TX(n) - TX(n-1),\quad X\in\{B,C\}$, for integer $n$ and $T$ being total.

    If the unit of action is continous, the value $MX$ will be given.
...
!!! info "Cost-benefit principle"
    ...
    - **Best action is when $ES$ is maximized**
