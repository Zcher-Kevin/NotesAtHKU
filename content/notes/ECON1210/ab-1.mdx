...
```math
\begin{array}{l}TB(n)=418n-7n^2\\MB(n)=425-14n\\AB(n)=418-7n\end{array}
```

Upon paying a monthly subscription fee of 45 dollars, Calvin will be eligible to watch any movie at P dollars per movie. If Calvin decides to watch 28 movies per month under the scheme, then we can infer that the price per movie (P) is less than or equal to X dollars and more than Y dollars.
...
Find opportunity cost by using maximum of T and W:

```math
\begin{align*}
OC(E) &= C(E) + \max\{ES(T),ES(W)\}\\
&= 210 + 10 + \max\{113\times1.5, 295\}\\
&= 210 + 10 + 295\\
&= 515
\end{align*}
```
...
First convert to marginal benefit table:
...
$72 \rightarrow 86 \rightarrow 94 \rightarrow 134 \rightarrow 144 \rightarrow 146\implies A1=1,B1=2,C1=3$

For (b), let $X=A2+B2+C2$:

```math
\begin{align*}
314.9\times X - f(X) &= 0,\quad\text{where f is the total minimum compensation}\\
X\to 10: 3149 - 1551 &= 1598\\
X\to 11: 3463.9 - 1822 &= 1641.9\\
X\to 12: 3778.8 - 2126 &= 1652.8 \\
X\to 13: 4093.7 - 2468 &= 1625.7\\
\end{align*}
```

$X=12\implies A2=4,B2=4,C2=4$
