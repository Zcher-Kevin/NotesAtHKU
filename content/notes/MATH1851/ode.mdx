---
title: Orinary Differential Equations
---
...
To find the general solution, we put the $\lambda$ roots into the **quadratic characteristic equation**:
    ...
    2. $\lambda_1=\lambda_2:\quad y=c_1\mathbf{e}^{\lambda x}+c_2x\mathbf{e}^{\lambda x}$
    3. $\{\lambda_{1,2}=\alpha\pm\beta i\}\in\mathbb{C}:\quad y=c_{1}\mathbf{e}^{\alpha x}\cos(\beta x)+c_{2}\mathbf{e}^{\alpha x}\sin(\beta x)$
...
2. $e^{\lambda x}\to x^\lambda,\quad x\to \ln x$
3. $e^{\alpha x}\to x^\alpha,\quad \beta x \to \beta \ln x$
</Block>
...
Where $a,b,c$ are constants. (Coefficients are *constants*)
<BlockSep />
We first solve for $\lambda_{1,2}$ for the [**complementary homogenous function**](#homogeneous-equations) $F_c$ to get $Y_c$:
```math
