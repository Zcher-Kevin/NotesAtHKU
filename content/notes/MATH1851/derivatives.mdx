...
$(u(v))'=u'(v)v'        \text{or}        \frac{dy}{dx}=\frac{dy}{du}\frac{du}{dx}$
</Block>


<Block variant="secondary" title="Product rule">
Shorthand: **d** from outside to inside
$(uv)' = uv' + vu'        \text{or}        \frac{d}{dx}(uv)=u\frac{dv}{dx}+v\frac{du}{dx}$
</Block>


<Block variant="secondary" title="Quotient rule">
Shorthand: move lower **d** upper - **d** lower x upper, lower square
$(\frac{u}{v})'=\frac{vu'-uv'}{v^2}        \text{or}        \frac{d}{dx}(\frac{u}{v})=\frac{v\frac{du}{dx}-u\frac{dv}{dx}}{v^2}$
</Block>
...
For any $a\in [\mathds{R}, \pm\infty]$, if $\lim_{x\to a}(\frac{f}{g})$ is in [indeterminate form](FIXME-sec:indeterminate) after substitution, we can conclude:
...
For $f(x)$ that is *continuous* in $[a,b]$ and *differentiable* in $(a,b)$:
$\forall\ f(a)=f(b)\text{  there exists  }f'(c)=0,     c\in(a,b)$
![](././img/rt.png)
<BlockSep />
...
| 10. | Area concavity? | $-++-/-+-+/$![](././img/arrow.png) |



Related: [Definition of asymptotes](FIXME-sec:asymptotes)
